...
-- ══════════════════════════════════════════════════════════════════════
--  apply_param_patch v3 — Task-Skill Binding Integrity
--
--  Milestone 3 新增：
--  · 需求 4：校验 task_run.skill_card_id === p_skill_card_id
--            （防止跨技能卡错误应用补丁）
--  · 需求 6：memory_episodes INSERT 补充 skill_history_id 列
--            （parameter_patch 必须记录 task_id/task_run_id/
--              skill_card_id/skill_history_id）
-- ══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF p_task_run_id IS NOT NULL THEN
    ...
    -- 需求 4：task_run.skill_card_id 必须与传入 p_skill_card_id 一致
    -- 允许两种合法情况：
    --   a) task_run.skill_card_id = p_skill_card_id（精确匹配）
    --   b) task_run.skill_card_id IS NULL（旧数据兼容，首次绑定场景）
    IF v_task_run.skill_card_id IS NOT NULL
       AND v_task_run.skill_card_id != p_skill_card_id THEN
      RAISE EXCEPTION
  ...
  --
  --  绑定字段（Milestone 3 需求 6）：
  --    列级：skill_card_id / task_id / task_run_id / skill_history_id（全部必填）
  --    JSON：同步写入 content_json 方便业务查询
  INSERT INTO memory_episodes (
    ...
    p_task_id,
    p_task_run_id,
    v_history_id,                     -- skill_history_id 列
...
COMMENT ON FUNCTION public.apply_param_patch IS
  '原子化应用白质层参数补丁（v3 – Milestone 3 Task-Skill Binding Integrity）。'
  '新增：① 校验 task_run.skill_card_id === p_skill_card_id（BINDING_MISMATCH 错误）；'
  '      ② memory_episodes.skill_history_id 列级写入（parameter_patch 四绑定完整）。'
  '事务保证：skill_cards UPDATE + skill_history INSERT + memory_episodes INSERT 全部在同一事务内。';
