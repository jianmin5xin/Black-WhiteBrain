...
-- ══════════════════════════════════════════════════════════════════════
--  evaluate_patch_outcome v5 — Milestone 7 需求 6
--
--  新增逻辑（相对 v4）：
--    · SELECT task_runs 时加入 is_legacy_run 列
--    · after_run.is_legacy_run = TRUE → 提前返回
--        evaluation_status = 'legacy_run_skipped'
--        ok = false
--        不写入任何 episode，不修改 skill_card 状态
--    · before_run.is_legacy_run = TRUE → 仅记录警告，降级为
--        evaluation_status = 'insufficient_data_before'（已有逻辑兼容）
--
--  所有其余逻辑与 v4 完全一致。
-- ══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ── 2. after run（含 is_legacy_run + snapshot，需求 4/6）──────────
  ...
  -- ── 3. before run（含 is_legacy_run + snapshot）───────────────────
  SELECT id, status, ended_at, duration_ms, tunable_params_snapshot, is_legacy_run
  INTO v_before_run
  FROM task_runs
  ...
  LIMIT 1;
  -- before_run 是 legacy 时仅降级为 insufficient_data_before（宽松处理）
...
COMMENT ON FUNCTION public.evaluate_patch_outcome(uuid, uuid, uuid) IS
  'evaluate_patch_outcome v5（Milestone 7 需求 6）：'
  'after_run.is_legacy_run=TRUE 时提前返回 legacy_run_skipped，不写入 episode；'
  'before_run 为 legacy 时降级 insufficient_data_before（宽松处理）；'
  '其余逻辑与 v4 完全一致。';
